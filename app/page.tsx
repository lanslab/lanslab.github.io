...
const deskLinks = [
  { href: "/work", code: "01", label: "Selected work", note: "Systems that scaled", className: "record-item record-work" },
  { href: "/experience", code: "02", label: "Experience", note: "TikTok / PwC / RBC", className: "record-item record-experience" },
  { href: "/profile", code: "03", label: "Profile", note: "Education / CHRP / Tools", className: "record-item record-profile" },
];
  ...
  return <main>
    ...
    <section className="home-hero shell">
      ...
      <aside className="record-display page-enter delay-one" aria-label="Explore Fanglan's portfolio">
        <div className="record-display-head">
          <span>Fanglan Shi, CHRP</span>
          <span>Career collection / 03</span>
        </div>
        ...
        <div className="record-display-foot">
          <span>Operate / Transform / Enable</span>
          <span>Choose a record</span>
        </div>
      </aside>
    </section>

    <section className="metric-band" aria-label="Impact at a glance">
      <div className="shell metric-grid">
        {metrics.map((metric) => <article key={metric.value}>
          <strong>{metric.value}</strong><p>{metric.label}</p><small>{metric.note}</small>
        </article>)}
      </div>
    </section>
    ...
    <section className="next-chapter shell"><p>Four systems that work better.</p><a href="/work">View selected work <span aria-hidden="true">→</span></a></section>
    <Footer />
  </main>;
}
